      ...
      <div class="col-sm-10">
        ...
        <table class="table table-sm">
          <thead class="thead- border-0">
            <tr><th>Comment</th><th>Rating</th><th>Reported by</th><th></th></tr>
          </thead>
          <tbody>
@foreach($comments as $i => $comment)
            <tr>
              ...
              <td>
@if($comment->approved)
                <a href="{{ route('backend.ratings.edit', $comment->id) }}" class="text-muted float-right">Edit/Hide comment</a>
@else
                <a href="{{ route('backend.ratings.edit', $comment->id) }}" class="text-muted float-right">Approve comment comment</a>
@endif
              </td>
            </tr>
@endforeach
