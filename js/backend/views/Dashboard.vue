<template>
  <div>
    <b-row>
      <b-col xl="6">
        <b-row
          v-if="this.$app.cmsEnabled && this.$app.user.can('view own posts')"
        >
          <b-col sm>
...
export default {
  ...
  async created() {
    if (this.$app.cmsEnabled && this.$app.user.can('view own posts')) {
      let { data } = await axios.get(this.$app.route('backend.posts.latest'))
      this.posts = data
    }
  }
}
